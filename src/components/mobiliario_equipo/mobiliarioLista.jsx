...
export default function InventarioMobiliarioEquipo() {
  ...
  const filtrados = useMemo(() => {
    ...
    return equipos.filter((equipo) =>
      [equipo.codificacion, equipo.marca, equipo.modelo, equipo.ubicacion, equipo.familia, equipo.estado]
        .join(" ")
        .toLowerCase()
        .includes(q)
    );
    ...
    <div className="h-full flex flex-col p-4">
      <div className="mb-4 rounded-2xl bg-white p-4 shadow-sm border border-slate-200">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900">Inventario de mobiliario y equipo</h1>
            <p className="text-sm text-slate-600">Control del inventario por familia y ubicación.</p>
          </div>
          <input
            value={busqueda}
            onChange={(e) => setBusqueda(e.target.value)}
            placeholder="Buscar por codificación, marca o ubicación"
            className="w-full md:w-80 rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-800"
          />
        </div>
      </div>
