...
export default function InventarioOtrosActivos() {
  ...
  const filtrados = useMemo(() => {
    ...
    if (!q) return equipos;
    return equipos.filter((equipo) => [equipo.codificacion, equipo.marca, equipo.modelo, equipo.ubicacion, equipo.familia, equipo.estado].join(" ").toLowerCase().includes(q));
  }, [equipos, busqueda]);
    ...
    <div className="h-full flex flex-col p-4">
      <div className="mb-4 rounded-2xl bg-white p-4 shadow-sm border border-slate-200">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900">Inventario de otros activos</h1>
            <p className="text-sm text-slate-600">Registros diversos no clasificados dentro de otras categorías.</p>
          </div>
          <input value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Buscar por codificación, marca o ubicación" className="w-full md:w-80 rounded-xl border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-800" />
        </div>
      </div>
            ...
            {loading ? (
              ...
              filtrados.map((equipo, index) => (
                <tr key={equipo.id ?? index} className="border-t border-slate-200 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">{equipo.estado || "-"}</td>
                  <td className="px-4 py-3">{equipo.ubicacion || "-"}</td>
                  <td className="px-4 py-3">{equipo.fechaIngreso ? new Date(equipo.fechaIngreso).toLocaleDateString("es-ES") : "-"}</td>
                </tr>
              ))
            ) : (
